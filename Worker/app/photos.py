...
def upload_save():
    ...
    try:
        ...
        if not os.path.isdir('app/static/user_images'):
            os.makedirs('app/static/user_images')
        ...
        s3_upload(fname, 1, new_file.filename, cursor, photo_id)
        print("original s3")
        # create thumbnail
        ...
        s3_upload(fname_thumbnail, 2, new_file.filename, cursor, photo_id)
        print("thumbdone")
        ...
        if os.path.isdir('app/static/user_images'):
            shutil.rmtree('app/static/user_images')
        print("Done ====================")
...
def create_and_store_transformation(img, cursor, trans_id, filename, photo_id):
    ...
    s3_upload(fname_transformation, trans_id, filename, cursor, photo_id)
    return


def s3_upload(tmp_path, trans_id, filename, cursor, photo_id):
    s3 = boto3.client('s3')

    if not s3.head_bucket(Bucket='ece1779cca2'):
        s3.create_bucket(Bucket='ece1779cca2')

    s3.upload_file(tmp_path, "ece1779cca2", session['username'] + '/' + str(trans_id) + "_" + filename)

    s3 = boto3.resource('s3')

    object_acl = s3.ObjectAcl('ece1779cca2', session['username'] + '/' + str(trans_id) + "_" + filename)
    object_acl.put(ACL='public-read')


    # Obtain url for public images
    url_s3 = 'https://s3.amazonaws.com/ece1779cca2/' + session['username'] + '/' + str(trans_id) + "_" + filename
    # store path to thumbnail
    query = "INSERT INTO transformation (filename,type_id,photo_id) VALUES (%s,%s,%s)"
    cursor.execute(query, (url_s3, trans_id, photo_id))